...
if (isset($_SESSION['ID']) && ($_SESSION['ID']['ROLE'] == 'admin')) {
    ...
    if (isset($_FILES["fileToUpload"]) && isset($_POST['id']) && isset($_POST['alt']) && isset($_POST["submit"])) {
        if (bateauMANAGER::confirmeDonneeImage($_POST['id']) == $_SESSION['ID']['ID']) {
            $target_dir = "../img/bateau/";
            if (file_exists($target_dir)) {
                $uploadOk = 1;
            ...
            $erreur = "Aucune modification autoriser sur ce groupe d'image " . $_POST['id'] . " " . $_SESSION['ID']['ID'] . " " . bateauMANAGER::confirmeDonneeImage($_POST['id']);
            $_SESSION['erreur'] = ['desc' => $erreur, 'code' => 50];
        }
    } else {
