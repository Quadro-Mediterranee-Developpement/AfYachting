...
<html>
    ...
    <nav class="navbar navbar-dark bg-primary nb-3">
        <a href="/Calendrier/index.php" class="navbar-brand">Mon calendrier</a>   
    </nav>
    ...
    <?php if (isset($_GET['day'])): ?>
        <div class="">
            ...
            $connex = new Calendar\Events(get_pdo());
            ?>
            <nav class="navbar   nb-3">
                <div class="calendar__weekday"><?= $month->days[$date->format('N') - 1]; ?> <?= $date->format('d'); ?> <?= $month->toString(); ?></div> 
                <div>
                    <a href="add.php?date=<?= $date->format('Y-m-d'); ?>" class="btn btn-primary">Ajouter un evenement</a>
                    <a href="/Calendrier/index.php?month=<?= $date->format('m'); ?>&year=<?= $date->format('Y'); ?>&day=<?= $date->format('d') - 1; ?>" class="btn btn-primary">&lt;</a>
                    <a href="/Calendrier/index.php?month=<?= $date->format('m'); ?>&year=<?= $date->format('Y'); ?>&day=<?= $date->format('d') + 1; ?>" class="btn btn-primary">&gt;</a>
                    <a href="/Calendrier/index.php?month=<?= $date->format('m'); ?>&year=<?= $date->format('Y'); ?>" class="btn btn-primary">retour</a>
                </div>
            </nav>
            <span class="block_note">
                <?php foreach ($eventsForDay as $event): ?>
                    <fieldset class="scheduler-border">
                        ...
                        <a href="/Calendrier/edit.php?id=<?= $event['id']; ?>"><h4><?= $event['name']; ?></h4></a>
                        <p>
                            <?= $event['description'] ?>
                            <?php
                            foreach (["Admin" => ["admin", "idAdmin"], "Client" => ["client", "idClient"], "Skipper" => ["skipper", "idSkipper"], "Client ponctuel" => ["client_ponctuel", "idClientTemp"]] as $k => $i) {
                                $name = $connex->findOneCompte($event[$i[1]], $i[0]);
                                echo "<div><strong>$k:</strong>$name</div>";
                            }
                            $name = $connex->findOneBoat($event['idBoat']);
    ...
    <?php else: ?>
        ...
        <nav class="navbar   nb-3">
            ...
            <div class="calendar">  
                ...
                <div>
                    <a href="/Calendrier/index.php?month=<?= $month->previousMonth()->month; ?>&year=<?= $month->previousMonth()->year; ?>" class="btn btn-primary">&lt;</a>
                    <a href="" class="btn btn-primary">mois</a>
                    <a href="/Calendrier/index.php?month=<?= $month->nextMonth()->month; ?>&year=<?= $month->nextMonth()->year; ?>" class="btn btn-primary">&gt;</a>

                    <a href="/Calendrier/index.php?month=<?= $month->month; ?>&year=<?= $month->year - 1; ?>" class="btn btn-primary">&lt;</a>
        ...
        <div class="d-flex flex-row align-items-center justify-content-between mx-sm-5">
                            ...
                            <td class="<?= $month->withinMonth($date) ? '' : "calendar__othermonth"; ?> <?= $eventsForDay ? 'is-event' : ''; ?> <?= $isToday ? 'is-today' : ''; ?>">
                                ...
                                foreach ($eventsForDay as $event):
                                    ...
                                    <div class="calendar__event">
                                        ...
                                        <a href="/Calendrier/edit.php?id=<?= $event['id']; ?>"><?= $event['name']; ?></a>
                                    </div>
                                    <?php
                                    $nbr++;
                                    if ($nbr > 1):
                                ...
                                ?>
                                <?php if ($nbr > 0): ?>
                                    <a href="/Calendrier/index.php?month=<?= $month->month; ?>&year=<?= $month->year; ?>&day=<?= $date->format('d'); ?>" class="calendar_day_more">voir plus</a>
                                <?php endif; ?>
                            </td>
                        <?php endforeach ?>
                    </tr>
            ...
            <a href="../Calendrier/add.php" class="calendar__button">+</a>
        <?php endif; ?>
    </div> 
</body>
</html>
